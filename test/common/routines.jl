...
const TARGET_PATCHES = Patch[
    # v1.13
    Patch(v"1.13.0-DEV.1250", # 0e30e0f39dd92a868373739cf8989c64dd29eaa2
        AddLatexSymbols("\\^q" => "𐞥")),
...
function beyond(version::VersionNumber)::Bool
    version > VERSION
end

function check_the_patches(f)
    up_to = first(TARGET_PATCHES).version
    patches = patches_to_be_loaded(; down_to = VERSION, up_to = up_to, patches = TARGET_PATCHES)
